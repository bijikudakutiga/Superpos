...
function seedData() {
  ...
  return { units: SEED_UNITS, suppliers, rawMaterials, recipes, batches: [], opnames: [], sales: [], purchases: [], pettyCash: [], auditLog: [], permissions: DEFAULT_PERMISSIONS, companyProfile: { name: "NAMA PERUSAHAAN ANDA", address: "Alamat perusahaan", phone: "08xx-xxxx-xxxx", email: "email@perusahaan.com" } };
}
...
export default function App() {
  ...
  function normalize(parsed) {
    return {
      ...
      companyProfile: { name: "NAMA PERUSAHAAN ANDA", address: "Alamat perusahaan", phone: "08xx-xxxx-xxxx", email: "email@perusahaan.com" },
      ...parsed,
      ...
      purchases: (parsed.purchases || []).map((p) =>
        p.items ? p : { ...p, items: p.rawMaterialId ? [{ rawMaterialId: p.rawMaterialId, name: p.name || "(bahan)", qty: p.qty, unitSymbol: p.unitSymbol, price: p.price }] : [] }
      ),
    };
  }
...
function generatePOPdf(po, supplier, company, unitById, showToast) {
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();

  doc.setFont("helvetica", "bold");
  doc.setFontSize(16);
  doc.text(company.name || "NAMA PERUSAHAAN", 14, 18);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.text(company.address || "-", 14, 24);
  doc.text(`Telp: ${company.phone || "-"}  Email: ${company.email || "-"}`, 14, 29);
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.text(`No: ${po.poNumber}`, pageWidth - 14, 25, { align: "right" });
  doc.text(`Tanggal: ${new Date(po.createdAt).toLocaleDateString("id-ID")}`, pageWidth - 14, 30, { align: "right" });
  ...
  doc.text(`Telp: ${supplier?.phone || "-"}`, 14, 53);
  ...
  autoTable(doc, {
    startY: 60,
    head: [["No", "Nama Bahan Baku", "Qty", "Satuan", "Harga Satuan", "Subtotal"]],
    body: rows,
  ...
  doc.setFontSize(9);
  doc.text("Dipesan oleh,", 14, finalY);
  doc.text("Disetujui oleh,", pageWidth - 60, finalY);
  doc.text("(_________________)", 14, finalY + 25);
  doc.text("(_________________)", pageWidth - 60, finalY + 25);
...
function generatePOExcel(po, supplier, company) {
  ...
  const aoa = [
    ...
    [],
    ["Kepada Yth:", supplier?.name || "-"],
    ["Telp:", supplier?.phone || "-"],
    [],
    ["No", "Nama Bahan Baku", "Qty", "Satuan", "Harga Satuan", "Subtotal"],
    ...po.items.map((it, idx) => [idx + 1, it.name, it.qty, it.unitSymbol, it.price, it.qty * it.price]),
    ["", "", "", "", "TOTAL", total],
  ];
...
function PurchasesPanel({ data, setData, unitById, logAction, showToast }) {
  ...
  const [items, setItems] = useState([{ id: uid("poi"), rawMaterialId: data.rawMaterials[0]?.id || "", qty: "", price: "" }]);
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [editingCompany, setEditingCompany] = useState(false);
  const [companyDraft, setCompanyDraft] = useState(data.companyProfile || { name: "", address: "", phone: "", email: "" });

  function addItemRow() {
    setItems([...items, { id: uid("poi"), rawMaterialId: data.rawMaterials[0]?.id || "", qty: "", price: "" }]);
  }
  function updateItem(id, patch) {
    setItems(items.map((i) => (i.id === id ? { ...i, ...patch } : i)));
  }
  ...
  function createPO() {
    ...
    for (const it of items) {
      if (!it.rawMaterialId || !it.qty || !it.price) return showToast("Lengkapi semua baris bahan baku (qty & harga)", true);
    }
    ...
    const po = {
      ...
      items: items.map((it) => {
        const rm = data.rawMaterials.find((r) => r.id === it.rawMaterialId);
        const purchaseUnit = unitById(rm.purchaseUnitId);
        return { rawMaterialId: it.rawMaterialId, name: rm.name, qty: parseFloat(it.qty), unitSymbol: purchaseUnit?.symbol, price: parseFloat(it.price) };
      }),
      status: "ordered",
      createdAt: new Date().toISOString(),
    };
    ...
    setItems([{ id: uid("poi"), rawMaterialId: data.rawMaterials[0]?.id || "", qty: "", price: "" }]);
  ...
  function receivePO(po) {
    ...
    for (const it of po.items) {
      ...
      if (!rm) continue;
      const purchaseUnit = unitById(rm.purchaseUnitId);
      const receivedBase = convertToBase(it.qty, purchaseUnit);
      updatedMaterials = updatedMaterials.map((r) =>
        r.id === rm.id
          ? { ...r, stock: r.stock + receivedBase, lastPrice: it.price, priceHistory: [{ price: it.price, date: new Date().toISOString() }, ...(r.priceHistory || [])].slice(0, 50) }
          : r
      );
    }
  ...
  function saveCompanyProfile() {
    setData({ ...data, companyProfile: companyDraft });
  ...
  return (
    <div>
      <SectionTitle
        action={
          <button style={btnGhost} onClick={() => setEditingCompany(!editingCompany)}><Settings size={14} />Info Perusahaan</button>
        }
      >
        Buat PO
      </SectionTitle>
      ...
      {editingCompany && (
        <div style={{ background: COLORS.panelLight, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 14, marginBottom: 16, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end" }}>
          ...
          <button style={btnPrimary} onClick={saveCompanyProfile}><Save size={14} />Simpan</button>
        </div>
      )}
      <p style={{ color: COLORS.muted, fontSize: 11.5, marginTop: -8, marginBottom: 16 }}>
        Info perusahaan ini dipakai sebagai kop surat saat generate dokumen PO untuk dikirim ke supplier.
      </p>
          ...
          {items.map((it) => {
            ...
            return (
              <div key={it.id} style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <input style={{ ...inputStyle, width: 90 }} type="number" placeholder="Jumlah" value={it.qty} onChange={(e) => updateItem(it.id, { qty: e.target.value })} />
                <input style={{ ...inputStyle, width: 120 }} type="number" placeholder="Harga/satuan" value={it.price} onChange={(e) => updateItem(it.id, { price: e.target.value })} />
            ...
            <div key={po.id} style={{ background: COLORS.panel, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: COLORS.muted }}>{po.items.length} item · Total Rp{total.toLocaleString("id-ID")}</div>
                </div>
              ...
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 3 }}>
                {po.items.map((it, idx) => (
                  <div key={idx} style={{ fontSize: 12, color: COLORS.muted, display: "flex", justifyContent: "space-between" }}>
                    <span>{it.name}</span>
                    <span style={{ fontFamily: "JetBrains Mono" }}>{it.qty} {it.unitSymbol} x Rp{it.price.toLocaleString("id-ID")}</span>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                <button style={btnGhost} onClick={() => generatePOPdf(po, sup, data.companyProfile || {}, unitById, showToast)}><Download size={14} />Surat PO (PDF)</button>
